...
public class VRLineRenderer : MonoBehaviour
{
    // Stored Line Data
    [SerializeField]
    protected Vector3[] m_Positions = new Vector3[0];
    ...
    const string k_RuntimeWidthChangeWarning =
        "Setting VRLineRenderer width at runtime should be done through SetWidth; this setter is intended for " +
        "editor use.";
    const string k_RuntimeColorChangeWarning =
        "Setting VRLineRenderer color at runtime should be done through SetColors; this setter is intended for " +
        "editor use.";
    ...
    {
        get { return m_WidthStart; }
        set
        {
            if (Application.isPlaying)
            {
                Debug.LogWarning(k_RuntimeWidthChangeWarning);
            }
            m_WidthStart = value;
        }
    }
    ...
    public Color colorStart
    {
        ...
        set
        {
            if (Application.isPlaying)
            {
                Debug.LogWarning(k_RuntimeColorChangeWarning);
            }
            m_ColorStart = value;
        }
    }
